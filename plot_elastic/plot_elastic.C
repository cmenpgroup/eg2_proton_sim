...
void plot_elastic(TString tar,Int_t r,Int_t iMax){
    ...
    TFile *outf = new TFile("plot_elastic_prunedC_01.root","RECREATE"); /* Save output plots in a root file */
    ...
    ntuple_sim->Draw("mc_P>>P_Ge(100,0,5)",total_cut_mc," ",nEvts);

    ntuple_sim->Draw("P>>P_Rec(100,0,5)",total_cut," ",nEvts);

    ntuple_sim->Draw("mc_Zh>>Zh_Gen(100,0,1.5)",total_cut_mc," ",nEvts);

    ntuple_sim->Draw("Zh>>Zh_Rec(100,0,1.5)",total_cut," ",nEvts);
    
    ntuple_sim->Draw("(mc_Eh+mc_Pz)/(0.938272+2*mc_Nu)>>Zlc_Gen(100,0,1.2)",total_cut_mc," ",nEvts);

    ntuple_sim->Draw("(Eh+Pz)/(0.938272+2*Nu)>>Zlc_Rec(100,0,1.2)",total_cut," ",nEvts);

    ntuple_sim->Draw("mc_Q2>>Q2_Gen(100,0,5)",total_cut_mc," ",nEvts);

    ntuple_sim->Draw("Q2>>Q2_Rec(100,0,5)",total_cut," ",nEvts);
    ...
    ntuple_sim->Draw("TMath::RadToDeg()*TMath::ATan2(mc_Py,mc_Px)>>PhiLab1_Gen(360,-180.0,180.0)",total_cut_mc," ",nEvts);

    ntuple_sim->Draw("TMath::RadToDeg()*TMath::ATan2(Py,Px)>>PhiLab1_Rec(360,-180.0,180.0)",total_cut," ",nEvts);

    ntuple_sim->Draw("TMath::RadToDeg()*TMath::ATan2(mc_Pey,mc_Pex)>>ePhiLab_Gen(360,-180.0,180.0)",total_cut_mc," ",nEvts);

    ntuple_sim->Draw("TMath::RadToDeg()*TMath::ATan2(Pey,Pex)>>ePhiLab_Rec(360,-180.0,180.0)",total_cut," ",nEvts);

    ntuple_sim->Draw("TMath::RadToDeg()*(TMath::ATan2(mc_Pey,mc_Pex)-TMath::ATan2(mc_Py,mc_Px))>>dPhiLab_Gen(360,-360.0,360.0)",total_cut_mc," ",nEvts);

    ntuple_sim->Draw("TMath::RadToDeg()*(TMath::ATan2(Pey,Pex)-TMath::ATan2(Py,Px))>>dPhiLab_Rec(360,-360.0,360.0)",total_cut," ",nEvts);
    
    ntuple_sim->Draw("mc_Mx2>>Mx2_Gen(100,-3.0,3.0)",total_cut_mc," ",nEvts);

    ntuple_sim->Draw("Mx2>>Mx2_Rec(100,-3.0,3.0)",total_cut," ",nEvts);
