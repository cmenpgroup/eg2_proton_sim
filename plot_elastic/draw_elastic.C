...
void draw_elastic(TString fAna="plot_elastic.root", TString targ="C", TString hprefix="P", TString xtitle="Momentum (GeV)")
{
    ...
    for(Int_t i=0; i<nSim; i++){
        ...
        h2D[i]->SetXTitle(xtitle);
        h2D[i]->GetXaxis()->CenterTitle();
        h2D[i]->SetYTitle("Counts");
        h2D[i]->GetYaxis()->CenterTitle();
        h2D[i]->GetYaxis()->SetTitleOffset(yoff);
        h2D[i]->Draw("colz");
    }
    TString OutCan = "draw_elastic_" + hprefix + "_" + targ +".gif";
	c1->Print(OutCan);
}
